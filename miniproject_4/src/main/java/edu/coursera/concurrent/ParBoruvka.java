...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        ParComponent candidateNode = null;

        while ((candidateNode = nodesLoaded.poll()) != null) {
            if (!candidateNode.lock.tryLock()) {
                // Optimistic Concurrency :: Current polled node is locked ,
                // continue and poll another node
                continue;
            }

            if (candidateNode.isDead) {
                candidateNode.lock.unlock();
                continue;
            }

            final Edge<ParComponent> edge = candidateNode.getMinEdge();
            if (edge == null) {
                // No Edge  -- > all nodes merged to MST graph
                solution.setSolution(candidateNode);
                break;
            }

            final ParComponent other = edge.getOther(candidateNode);

            // Try to get lock on Other node so no other thread is also Merging with this Node
            if (!other.lock.tryLock()) {
                candidateNode.lock.unlock();
                nodesLoaded.add(candidateNode);
                continue;
            }
            ...
            candidateNode.merge(other, edge.weight());
